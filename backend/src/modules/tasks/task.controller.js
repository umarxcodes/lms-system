import { createTask, getAllTasks, getTaskById, updateTaskStatus, assignTask } from "./task.service.js";
...
export const updateTaskStatusController = async (req, res, next) => {
  try {
    ...
    success(res, task, "Task status updated");
  } catch (err) {
    next(err);
  }
};
...
export const assignTaskController = async (req, res, next) => {
  try {
    ...
    success(res, task, "Task assigned");
  } catch (err) {
    next(err);
  }
};
